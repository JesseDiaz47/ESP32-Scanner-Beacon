...
`data/` and the Wi-Fi and Bluetooth screenshots contain a real capture from one location, with the identifier columns pseudonymised. That split is deliberate: a survey tool documented with invented numbers demonstrates nothing, but the identifier columns were other people's, and an SSID set can locate a capture with no GPS attached. Counts, channels, RSSI, hidden-network totals and company IDs are exactly as observed. SSIDs read `AP-nn` and BLE names read `Device-n`, stable across sweeps. BLE addresses are published as captured where the address rotates on its own — 18 of 28, classified by the two most significant bits per Core spec Vol 6 Part B 1.3 — and the remaining 10, which are static-random or public, are replaced with stand-ins of the same address class, so the published distribution stays truthful. Mapping tables are not committed. It is a single snapshot with no location tags, no traffic, and no repeated readings. Publishing a *walk* — repeated readings tied to named places — is a different decision and has not been made here.
